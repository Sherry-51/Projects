...
void OrderQueue::enqueue(const char* itemName, const char* spiceLevel, const char* size, double price) {
    ...
    if (rear == NULL) {
        front = rear = newNode;
        cout << "\n*** Order #" << newNode->order.orderNumber << " placed successfully! ***\n";
        return;
    }
...
void RestaurantSystem::placeOrder() {
    ...
    if (item->price <= 0) {
        cout << "\nThis is a category, not an item. Please select an actual food item.\n";
        return;
    }
    // Get spice level
    char spiceLevel[20];
...
void RestaurantSystem::displayOrderDetails() {
